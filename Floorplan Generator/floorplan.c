#include "floorplan.h"
/* The pupose of this MP is to follow a set of structs to design a slicing floorplanner. We first create an initial slicing tree,
    to create a horizontally-aligned floorplan. We then determine some properties of the tree (if its a leaf, in a subtree, an internal node etc.).
    We then convert the 2D floorplan into a 1D floorplan by slicing the tree depending on its properties. This method is used for optimization when
    the chip size is very small. 
*/
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  else if (a==NULL) return ret;
  else if (is_in_subtree(a->left,b)||is_in_subtree(a->right,b)){
    ret=1;
    return ret;
  }
  else  return ret;
...
void recut(node_t* ptr) {
  ...
  else if(ptr->cutline==V) ptr->cutline=H;
...
void swap_module(node_t* a, node_t* b) {
  ...
  t=a->module;
  a->module=b->module;
  b->module=t;  //Swaping the modules
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  if (a->parent->right==a) temp=a->parent->right;

  else if(a->parent->left==a) temp=a->parent->left;

  if(b->parent->right==b){
    if(a->parent->left==a)  a->parent->left=b->parent->right;
    else if(a->parent->right==a)  a->parent->right=b->parent->right;

    b->parent->right=temp;
  }

  else if(b->parent->left==b){
    if(a->parent->left==a)  a->parent->left=b->parent->left;
    else if(a->parent->right==a)  a->parent->right=b->parent->left;
...
int get_total_resource(node_t* ptr)
{
  // TODO:
  int sum_resources=0;
  for(int i=0;i<num_modules;i++)  sum_resources+=modules[i].resource;
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  if(n==(num_modules-1)){
    par=(node_t*)malloc(sizeof(node_t));  //Allocating memory for par
    par->right=NULL;
    par->left=NULL;
    ...
    return par;
  } //if ends setting values for par
